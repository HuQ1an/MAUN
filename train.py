...
tensorboard_path = opt.outf + date_time + '/runs/'
...
# resume
#optimizer = torch.optim.Adam([{'params': model.parameters(), 'initial_lr': 4e-4}], lr=opt.learning_rate,betas=(0.9, 0.999))
if opt.scheduler=='MultiStepLR':
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=opt.milestones, gamma=opt.gamma)
elif opt.scheduler=='CosineAnnealingLR':
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, opt.max_epoch, eta_min=1e-6)
    # resume
    #scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, opt.max_epoch, eta_min=1e-6,last_epoch = 237)
...
def train(epoch, logger):
    ...
    for i in range(batch_num):
        ...
        if opt.method in ['cst_s', 'cst_m', 'cst_l']:
            ...
            # input_meas : y  #input_mask_train phi,phi_s
        ...
        #endb = time.time()
        #logger.info("===> Epoch {} Complete:  time: {:.2f}".format(epoch, (endb - beginb)))     
                        
    end = time.time()
    logger.info("===> Epoch {} Complete: Avg. Loss: {:.6f} time: {:.2f}".
                format(epoch, epoch_loss / batch_num, (end - begin)))
    writer.add_scalar('scalar/train_loss', epoch_loss / batch_num, epoch)
    return 0
...
def test(epoch, logger):
    ...
    writer.add_scalar('scalar/test_psnr', psnr_mean, epoch)
    writer.add_scalar('scalar/test_ssim', ssim_mean, epoch)
    model.train()
